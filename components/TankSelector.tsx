...
export default function TankSelector({ value, onChange, label = 'Estanque' }: Props) {
  ...
  useEffect(() => {
    supabase
      .from('tanks')
      .select('id, code, name, current_liters')
      .eq('status', 'activo')
      .order('name')
      .then(({ data }) => {
      ...
      >
        {loading ? (
          ...
          <Text style={selected ? styles.selectedText : styles.placeholder}>
            {selected
              ? `${selected.code} — ${selected.name} (${selected.current_liters.toLocaleString('es-CL')} L)`
      ...
      <Modal visible={modalVisible} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>Seleccionar estanque</Text>
            <FlatList
              data={tanks}
              ...
              renderItem={({ item }) => (
                ...
                >
                  ...
                  <Text style={styles.optionLiters}>
                    {item.current_liters.toLocaleString('es-CL')} litros disponibles
                  </Text>
                </TouchableOpacity>
              )}
...
const styles = StyleSheet.create({
  ...
  optionLiters: { fontSize: 13, color: '#888' },
